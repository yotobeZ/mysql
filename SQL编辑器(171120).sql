...
CREATE TRIGGER  `学生管理`.`TT4` before INSERT ON  `学生管理`.`出库表`
 FOR EACH ROW BEGIN
  ...
  set @rusj=NOW();
  set @rksl=new.出库数量;
 set @kcl=(SELECT  库存量  FROM 库存表 WHERE 货物号=@hwh);
  IF EXISTS(SELECT * FROM 库存表 WHERE 货物号=@hwh)&&((@rksl-@kcl)<0)
  then
     UPDATE 库存表 SET 库存量=库存量-@rksl,更新日期=@rusj WHERE 货物号=@hwh;
     else  delete from `学生管理`.`出库表` where  货物号=@hwh and 出库时间=@rusj;
END IF;
end;
/*出错：Can't update table 'psxt_user' in stored function/trigger
原因是MYSQL中触发器中不能对本表进行 insert ,update ,delete 操作，以免递归循环触发。*/
...
CREATE TRIGGER  `学生管理`.`TT5` before update ON  `学生管理`.`出库表`
 FOR EACH ROW BEGIN
  ...
  set @kcl=(SELECT  库存量  FROM 库存表 WHERE 货物号=@hwh);
   IF ((@rksl2-@kcl)<0) then
  UPDATE 库存表 SET 库存量=库存量+@rksl1-@rksl2,更新日期=@rusj WHERE 货物号=@hwh ; 
  else UPDATE 出库表  set 出库数量=@rksl1  where  货物号=@hwh and 出库时间=@rusj;
  end if;
END;
